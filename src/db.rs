...
impl JiraDatabase {
    ...
    pub fn create_epic(&self, epic: Epic) -> Result<u32> {
        let mut state = self.read_db()
            .map_err(|_|anyhow!("Unable to read from database when attempting to create new Epic"))?;

        state.new_epic(epic);

        self.database.write_db(&state)
            .map_err(|_| anyhow!("Unable to write to database after creating new Epic, Epic with ID: {}", state.last_item_id))?;
    ...
    pub fn create_story(&self, story: Story, epic_id: u32) -> Result<u32> {
        let mut state = self.read_db()
            .map_err(|_|anyhow!("Unable to read from database when attempting to create new Story for Epic with ID: {epic_id}"))?;

        state.new_story(story, epic_id)?;

        self.database.write_db(&state)
            .map_err(|_| anyhow!("Unable to write to database after creating new Story, Story with ID: {}", state.last_item_id))?;
    ...
    pub fn delete_epic(&self, epic_id: u32) -> Result<()> {
        let mut state = self.read_db()
            .map_err(|_|anyhow!("Unable to read from database while attempting to delete Epic with ID: {epic_id}"))?;

        state.delete_epic(epic_id)?;

        self.database.write_db(&state)
            .map_err(|_| anyhow!("Unable to write to database after deleting Epic, ID: {epic_id}"))?;
    ...
    pub fn delete_story(&self,epic_id: u32, story_id: u32) -> Result<()> {
        let mut state = self.read_db()
            .map_err(|_|anyhow!("Unable to read from database while attempting to delete Story with ID: {story_id}"))?;

        state.delete_story(story_id, epic_id)?;

        self.database.write_db(&state)
            .map_err(|_| anyhow!("Unable to write to database after deleting Story, ID: {story_id}"))?;
    ...
    pub fn update_epic_status(&self, epic_id: u32, status: Status) -> Result<()> {
        let mut state = self.read_db()
            .map_err(|_|anyhow!("Unable to read from database while attempting to update Epic with ID: {epic_id}"))?;

        state.update_epic_status(epic_id, status)?;

        self.database.write_db(&state)
            .map_err(|_| anyhow!("Unable to write to database after updating Epic, ID: {epic_id}"))?;
    ...
    pub fn update_story_status(&self, story_id: u32, status: Status) -> Result<()> {
        let mut state = self.read_db()
            .map_err(|_|anyhow!("Unable to read from database while attempting to update Story with ID: {story_id}"))?;

        state.update_story_status(story_id, status)?;

        self.database.write_db(&state)
            .map_err(|_| anyhow!("Unable to write to database after updating Story, ID: {story_id}"))?;
...
impl Database for JSONFileDatabase {
    ...
    fn write_db(&self, db_state: &DBState) -> Result<()> {
        // let output = serde_json::to_string_pretty(db_state)?;
        ...
        fs::write(&self.file_path, &serde_json::to_string_pretty(db_state)?)?;
        Ok(())
    }
}

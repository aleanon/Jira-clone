...
impl Navigator {
    pub fn new(db: Rc<JiraDatabase>) -> Self {
        Self { pages: vec![Box::new(HomePage {db: db.clone()})], prompts: Prompts::new(), db, }
    }
    ...
    pub fn handle_action(&mut self, action: Action) -> Result<()> {
        match action {
            Action::NavigateToEpicDetail { epic_id } => {
                let details = EpicDetail {
                    db: self.db.clone(), 
                    epic_id
                };

                self.pages.push(Box::new(details));
            ...
            Action::DeleteEpic { epic_id } => {
                if let true = (&self.prompts.delete_epic)() {
            ...
            Action::DeleteStory { epic_id, story_id } => {
                if let true = (&self.prompts.delete_story)() {

...
impl Page for HomePage {
    fn draw_page(&self) -> Result<()> {
        ...
        for (id, epic) in db.epics.iter().sorted_unstable_by_key(|(id, _)| *id) {
            println!("{}|{}|{}", 
                ...
                match n.parse::<u32>() {
                    Ok(epic_id) => {
                        ...
                        match db.epics.get(&epic_id) {
                            Some(_) => Ok(Some(Action::NavigateToEpicDetail { epic_id, })),
                            None => Ok(None),
                        }
                    }
                    Err(_) => Ok(None),
...
impl Page for EpicDetail {
    fn draw_page(&self) -> Result<()> {
        ...
        for id in &epic.stories {
            let story = db_state.stories.get(&id).expect("Invalid Story ID");
            println!("{}|{}|{}", 
                get_column_string(&id.to_string(), 12), 
        ...
        match input {
            ...
            n => {
                match n.parse::<u32>() {
                    Ok(story_id) => {
                        let db = self.db.read_db()?;
                        let epic = db.epics.get(&self.epic_id).expect("Trying to access invalid Epic");
